...
def create_pipeline():
    try:
        ...
        if(not params['pipeline_name'] or not params['repo_url'] or not params['jenkinsfile_path'] or not params['owner_id']):
            return resp(400, "check your values")
        
        user_match = User.query.filter(and_(User.id == params['owner_id'], User.deleteAt == None)).first()
        if(user_match):
            result = Pipeline(params['pipeline_name'], params['repo_url'], params['jenkinsfile_path'], params['owner_id'])
            db_apply([result])
